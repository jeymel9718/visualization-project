...
export const accentColor = '#ECFAFA';
export const background = '#3B7875';
export const background2 = '#288B86';
...
const getDate = (d: AppleStock) => {
    const [monthStr, yearStr] = d.date.split(' ');
...
function BrushChart({
  ...
  const stockScale = useMemo(
    () =>
      scaleLinear<number>({
        range: [yMax, 0],
        domain: [0, Math.max(...filteredStock.map(getStockValue)) || 0],
        nice: true,
      }),
    [yMax, filteredStock],
  );
  ...
  const initialBrushPosition = useMemo(
    () => ({
      ...
      end: { x: brushDateScale(getDate(data[30])) },
    }),
    [brushDateScale],
  );
